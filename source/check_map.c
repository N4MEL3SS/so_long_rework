...
{
	//if (err->way)
	// 	terminate(ERR_MAP_PASS, NULL); //TODO: Проработать поиск путей
	if (err->rowlen)
...
void	check_line(char *line, t_mchk *op, t_merr *err)
{
	...
	if ((op->fl_line && op->char_arr[WALL_INDEX] != op->col - 1) || \
		...
		err->borders = 1;
	err->n_collect = op->char_arr[COLLECT_INDEX] < 1;
...
void	check_map(int *fd, t_mchk *opt, t_map *map)
{
	...
	i = 0;
	map->map_count = fd[0] - 1;
	while (++i < fd[0])
	{
		map_opt_init(opt);
		map_err_init(&err);
		read_col_row(fd[i], opt, &err);
		print_map_error(&err);
		map->col[i - 1] = opt->col - 1;
		map->row[i - 1] = opt->row;
	}
	free(fd);
}
